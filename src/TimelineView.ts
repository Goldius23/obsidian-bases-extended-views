...
export class TimelineView extends Component {
  ...
  private calculateDateRange(
    ...
    let start = midnight(
      new Date(minTime - this.getPadding() * 86400000)
    );
    let end = midnight(
      new Date(maxTime + (this.getPadding() + 1) * 86400000)
    );
  ...
  render() {
    ...
    header.createDiv("btl-header-spacer").style.width =
      `${sidebarWidth}px`;
  ...
  private renderToolbar(sidebarWidth: number) {
    ...
    bar.createDiv("btl-toolbar-spacer").style.width =
      `${sidebarWidth}px`;
